...
class HKEnv(gym.Env):
    ...
    HP_CKPT = np.array([55,56,57,58,59, 94, 133, 171, 207, 242, 278, 314, 352], dtype=int)
    #HP_CKPT = np.array([53,52,51,50,54, 55,56,57,58,59], dtype=int)
    #HP_CKPT = np.array([55,56,57,58,59], dtype=int)
    ...
    def observe(self, knight_only=False):
        ...
        if knight_only:
            ...
            enemy_hp_bar = frame[625, 181:769, :3]
            enemy_hp_bar_red = enemy_hp_bar[..., 2]
            enemy_hp = (((enemy_hp_bar_red - enemy_hp_bar[..., 0]) == 201)
                        & ((enemy_hp_bar_red - enemy_hp_bar[..., 1]) == 209))
            first_pixel_hp = (frame[625,181,2]==198 & frame[625,181,0]==88 & frame[625,181,1]==83)
            second_pixel_hp = (frame[625,182,2]==237 & frame[625,182,0]==39 & frame[625,182,1]==31)
        ...
        checkpoint1 = knight_hp_bar[self.HP_CKPT[5:]]         
        checkpoint2 = knight_hp_bar[self.HP_CKPT[5:] - 1]

        checkpoint3 = knight_hp_bar[self.HP_CKPT[:5]]    
        checkpoint4 = knight_hp_bar[self.HP_CKPT[:5] - 1]      
        first_point_hp = ((checkpoint3 > 180) | (checkpoint4 > 180)).sum()   
        knight_hp = ((checkpoint1 > 180) | (checkpoint2 > 180)).sum()
